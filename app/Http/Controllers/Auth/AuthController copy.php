...
class AuthController extends Controller
{
    ...
    public function storeRegister(RegisterRequest $request)
    {
        ...
        try {
            // Simpan password plaintext untuk WA
            $validated['password_plaintext'] = $validated['password'];
        ...
        /**
         * ===========================
         * KIRIM OTP (SETELAH COMMIT)
         * ===========================
         */
        $sendVerificationCodeResponse = $this->sendVerificationCode($user, true);
        $verificationCode = $sendVerificationCodeResponse->data['code'] ?? null;

        if ($verificationCode) {
            try {
                $this->sendVerificationCodeWhatsApp($user, $verificationCode);
            } catch (\Throwable $e) {
                Log::error('WA FAILED', [
                    'user_id' => $user->id,
                    'error' => $e->getMessage()
    ...
    public function viewVerifyAccount(string $user)
    {
        ...
        // ❗ Pakai verified_at, bukan email_verified_at
        if (!is_null($getUserResponse->data->verified_at)) {
    ...
    public function storeVerifyAccount(Request $request, string $user)
    {
        ...
        try {
            ...
            return redirect()
                ->route('auth.login')
                ->with('toastSuccess', __('auth.activated'));
        } catch (\Throwable $th) {
        ...
        try {
            ...
            if ($withRefreshCode) {
                $user->update([
                    ...
                    'verified_at' => null   // ❗ reset status verifikasi
                ]);
            }
            ...
            try {
                ...
                \Log::error('WA verify failed: ' . $waException->getMessage());
            }
