...
function setupResponsiveTables() {
    document.querySelectorAll(".table-responsive > table.table, .table-responsive table.table").forEach((table) => {
        if (table.dataset.responsiveCard === "false" || table.closest(".mahasiswa-table-desktop")) {
            return;
        }

        const headers = Array.from(table.querySelectorAll("thead th")).map((header) => {
            return header.innerText.replace(/\s+/g, " ").trim();
        });
        ...
        table.closest(".table-responsive")?.classList.add("responsive-card-table-wrapper");
        ...
        table.querySelectorAll("tbody tr").forEach((row) => {
            Array.from(row.children).forEach((cell, index) => {
                ...
                if (label && !cell.hasAttribute("colspan")) {
